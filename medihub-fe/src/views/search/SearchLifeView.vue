...
const fetchSearchResultList = async () => {
  ...
  try {
    ...
    resultList.value = response.data.data.map(life => ({
      ...life,
      isLiked: false,
      isBookmark: false
    }));
    await Promise.all(resultList.value.map(result => {
      ...
      return Promise.all([updateLikeStatus(result), updateBookmarkStatus(result)]);
    }));
  } catch(error) {
...
const sortedResultList = computed(() => {
  ...
  return resultList.value.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5);
});
...
const updateLikeStatus = async (result) => {
      ...
      <ul>
        <li class="result-item" v-for="result in sortedResultList" :key="result.id">
          <div class="result-content">
            <span
                class="result-title"
                @click="goToDetail(result.medicalLifeSeq)"
          ...
          <span class="result-icons">
            <span class="like-btn" @click.stop="toggleLike(result)">
