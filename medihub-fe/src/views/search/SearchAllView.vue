...
const resultCaseList = ref([]);
const resultLifeList = ref([]);
const resultBoardList = ref([]);
const categories = [
...
const fetchSearchResultCaseList = async () => {
  ...
  try {
    ...
    resultCaseList.value = response.data.data.map(cases => ({
      ...cases,
      isLiked: false,
      isBookmark: false
    }));
    // 각 케이스 공유의 좋아요 및 북마크 상태 업데이트
    await Promise.all(resultCaseList.value.map(result => {

      return Promise.all([updateCaseBookmarkStatus(result)]);
    }));
  } catch(error) {
...
const fetchSearchResultLifeList = async () => {
  ...
  try {
    ...
    resultLifeList.value = response.data.data.map(life => ({
      ...life,
      isLiked: false,
      isBookmark: false
    }));
    // 각 메디컬 라이프의 좋아요 및 북마크 상태 업데이트
    await Promise.all(resultLifeList.value.map(result => {

      return Promise.all([updateLifeLikeStatus(result), updateLifeBookmarkStatus(result)]);
    }));
  } catch(error) {
...
const fetchSearchResultBoardList = async () => {
  ...
  try {
    ...
    resultBoardList.value = response.data.data.map(board => ({
      ...board,
      isLiked: false,
      isBookmark: false
    }));
    // 각 익명 게시글의 좋아요 및 북마크 상태 업데이트
    await Promise.all(resultBoardList.value.map(result => {

      return Promise.all([updateBoardLikeStatus(result), updateBoardBookmarkStatus(result)]);
    }));
  } catch (error) {
...
const goToLifeList = () => {
  ...
  router.push({ name: 'medical_Life' });
};

const goToLifeDetail = (id) => {
...
const goToBoardList = () => {
  ...
  router.push({ name: 'AnonymousBoardList' });
};
...
const toggleCaseBookmark = async (result) => {
...
const updateCaseBookmarkStatus = async (result) => {
...
const toggleLifeLike = async (result) => {

  const medicalLifeSeq = result.medicalLifeSeq;

  try {

    const response = await axios.patch(`/medical-life/${medicalLifeSeq}/prefer`);

    result.isLiked = response.data.data;

    alert(result.isLiked ? '좋아요가 등록되었습니다.' : '좋아요가 취소되었습니다.');
  } catch(error) {
...
const toggleLifeBookmark = async (result) => {
...
const updateLifeLikeStatus = async (result) => {

  try {

    const response = await axios.get(`/medical-life/${result.medicalLifeSeq}/prefer`);

    // 응답이 존재하고, 데이터가 있으면 좋아요 상태 설정
    result.isLiked = response.data?.data ?? false;
  } catch(error) {
...
const updateLifeBookmarkStatus = async (result) => {

  try {

    const response = await axios.get(`/medical-life/${result.medicalLifeSeq}/bookmark`);
...
const toggleBoardLike = async (result) => {

  const anonymousBoardSeq = result.anonymousBoardSeq;
...
const toggleBoardBookmark = async (result) => {
...
const updateBoardLikeStatus = async (result) => {

  try {

    const response = await axios.get(`/anonymous-board/${result.anonymousBoardSeq}/prefer`);
...
const updateBoardBookmarkStatus = async (result) => {

  try {

    const response = await axios.get(`/anonymous-board/${result.anonymousBoardSeq}/bookmark`);
...
onMounted(() => {
  ...
  fetchSearchResultCaseList();
  fetchSearchResultLifeList();
  fetchSearchResultBoardList();
  fetchKeywordList();
});
</script>
  ...
  <div class="result-list-page">
    ...
    <div class="search-result-section">
      ...
      <div class="total-result-list">
        <p>전체 : <span class="total-result-list-highlight">{{ resultCaseList.length }}</span>건</p>
      </div>

      <ul>
        <li class="result-item" v-for="result in sortedResultCaseList" :key="result.id">
          <div class="result-content">
            <span
                class="result-title"
                @click="goToCaseDetail(result.caseSharingSeq)"
          ...
          <span class="result-icons">
            <span class="bookmark-btn-case" @click.stop="toggleCaseBookmark(result)">
              <img
      ...
      <div class="more-result-list" v-if="resultCaseList.length > 5">
        <p @click="goToCaseList()" style="cursor: pointer;">
          + 결과 더 보기 [<span class="highlight">{{ resultCaseList.length }}</span>건]
        </p>
      ...
      <div class="total-result-list">
        <p>전체 : <span class="total-result-list-highlight">{{ resultLifeList.length }}</span>건</p>
      </div>

      <ul>
        <li class="result-item" v-for="result in sortedResultLifeList" :key="result.id">
          <div class="result-content">
            <span
                class="result-title"
                @click="goToLifeDetail(result.medicalLifeSeq)"
                style="cursor: pointer;"
            >
              {{ result.medicalLifeTitle }}
            </span>
          </div>
          ...
          <span class="result-user">{{ result.partName }}</span>
          <span class="result-user">{{ result.userName }} {{ result.rankingName }}</span>

          <span class="result-date"><LocalDateTimeFormat :data="result.createdAt"/></span>

          <span class="result-icons">
            <span class="like-btn" @click.stop="toggleLifeLike(result)">
              <img :src="result.isLiked ? afterLike : beforeLike" :alt="result.isLiked ? 'After' : 'Before'"/>
            </span>

            <span class="bookmark-btn" @click.stop="toggleLifeBookmark(result)">
              <img
                  :src="result.isBookmark ? afterBookmark : beforeBookmark"
                  :alt="result.isBookmark ? 'After' : 'Before'"
              />
      ...
      <div class="more-result-list" v-if="resultLifeList.length > 5">
        <p @click="goToLifeList()" style="cursor: pointer;">
          + 결과 더 보기 [<span class="highlight">{{ resultLifeList.length }}</span>건]
        </p>
        <!--        <p>MEDICAL LIFE로 이동하기 →</p>-->
      </div>
      ...
      <div class="total-result-list">
        <p>전체 : <span class="total-result-list-highlight">{{ resultBoardList.length }}</span>건</p>
      </div>

      <ul>
        <li class="result-item" v-for="result in sortedResultBoardList" :key="result.id">
          <div class="result-content">
            <span
                class="result-title"
                @click="goToBoardDetail(result.anonymousBoardSeq)"
                style="cursor: pointer;"
            >
              {{ result.anonymousBoardTitle }}
            </span>
          </div>
          ...
          <span class="result-user">익명</span>

          <span class="result-date"><LocalDateTimeFormat :data="result.createdAt"/></span>

          <span class="result-icons">
            <span class="like-btn" @click.stop="toggleBoardLike(result)">
              <img :src="result.isLiked ? afterLike : beforeLike" :alt="result.isLiked ? 'After' : 'Before'"/>
            </span>

            <span class="bookmark-btn" @click.stop="toggleBoardBookmark(result)">
              <img
                  :src="result.isBookmark ? afterBookmark : beforeBookmark"
                  :alt="result.isBookmark ? 'After' : 'Before'"
              />
      ...
      <div class="more-result-list" v-if="resultBoardList.length > 5">
        <p @click="goToBoardList()" style="cursor: pointer;">
          + 결과 더 보기 [<span class="highlight">{{ resultBoardList.length }}</span>건]
        </p>
<!--        <p>ANONYMOUS BOARD로 이동하기 →</p>-->
      </div>
    </div>
  </div>
</template>
