  ...
  const isCreatingNewData = ref(true);        // 생성 여부 확인 변수
  ...
  async function fetchCpSearchCategoryDataData(cpSearchCategorySeq) {
    try {
      ...
      // cpSearchCategorySeq를 문자열로 변환하여 match 메서드 호출
  ...
  function findSelectedCategory() {
    ...
    const selectedCategory =
        cpSearchCategoryList
            .value
            .find(category => category.cpSearchCategoryName === selectedOption.value);
