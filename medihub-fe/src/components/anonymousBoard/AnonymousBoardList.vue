...
const sortedBoardList = computed(() => {
  ...
  return [...(props.searchResult.length > 0 ? props.searchResult : boardList.value)].sort((a, b) => {
    ...
    } else if(props.sortOption === '조회순') return b.anonymousBoardViewCount - a.anonymousBoardViewCount; // 내림차순 정렬
  ...
  <div class="board-list">
    <table>
      ...
      <tr v-for="(boardItem) in paginatedBoards" :key="boardItem.anonymousBoardSeq">
        ...
        <td>
          <router-link :to="`/anonymous-board/${boardItem.anonymousBoardSeq}`">
            {{ boardItem.anonymousBoardTitle }}
          </router-link>
        </td>
        <td>{{ boardItem.keywords ? boardItem.keywords.join(', ') : '' }}</td>
